// ALGORYTM BELLMANA_FORDA MACIERZOWO
...
/************** PSEUDOKOD BELLMANA-FORDA Z KTOREGO KORZYSTAM :
Bellman-Ford(G,w,s):
...
void Algorytm_Bellmana(std::vector<int> & P ,int n,int pocz) {

	const int inf = std::numeric_limits<int>::max();


	int x,y; // wierzcholek x rozpatrywany, y to jego sasiedzi 
	
	D.clear();  // testujemy kilka razy program
	P.clear();
	...
	for(int i=1 ; i < n ; i++ ){   // petla n-1 razy, dla kazdego wierzcholka 


		for(x=0;x<n;x++){
               for(y=0 ; y < n; y++){
				   if(Macierz[x][y]!=0){         // dla kazdej krawedzi
					   if(  (D[y] > ( D[x] + Macierz[x][y])) ){
							D[y]=(D[x]+Macierz[x][y]);
							P[y]=x;
					   }
				   }
	...
	/*
	cout<<"Wyswietlam rezultaty dzialania algorytmu Forda- Bellmana  : "<<endl;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cout<<"Program - Algorytm Bellmana-Forda - macierz sasiedztwa \n"<<endl;
	...
	G.Algorytm_Bellmana(P,n,0); // 0- zrodlo
